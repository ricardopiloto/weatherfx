...
## Non-legacy Weather Control (DSLF)

When Weather Control has **`legacyEnemyInShadowsWeather`** set to **`false`**, chat lines look like `<b>15 °C</b> - Light; Clear; Strong` (precipitation; visibility; wind). Weather FX maps that triple to FXMaster effects.

- **Clear** visibility: fog layers from rain presets are **not** applied (so Clear air is not misty). If the effect has no clouds yet, a **few sparse clouds** are added so wind strength is visible; wind still scales cloud speed (Still slow → Strong faster).
- Example: `Light; Clear; Strong` → light rain, no fog, sparse fast-moving clouds (not heavy rain / thick mist).
...
Publish a **GitHub Release** (tag, e.g. `v1.2.2`). CI substitutes `${version}`, `${url}`, `${manifest}`, and `${download}` in `module.json`, then uploads `weatherfx.zip` and the filled manifest — no manual download URL edits.
